...
export default function MobileMenu({
  ...
  return (
    <div
      className={`bg-bg fixed top-17 right-0 bottom-0 left-0 z-50 flex flex-col items-center justify-center p-6 transition-all ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}
    >
      <ul className="font-family-display mb-10 flex flex-col items-center gap-10 text-2xl font-bold">
        {menuItems.map((item) => (
          <li key={item.anchor}>
            <a href={item.anchor} onClick={onNavigate}>
              {item.title}
            </a>
          </li>
        ))}
